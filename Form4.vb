...
Public Class Form4
    ...
    Private Sub Button2_Click(sender As Object, e As EventArgs) Handles Button2.Click
        ' SQL query to select staff information based on staff ID
        Dim sql As String = "SELECT * FROM staff WHERE staffid = @staffid"

        ' Create a list for SQL parameters
        Dim sqlParameters As New List(Of SqlParameter) From {
        New SqlParameter("@staffid", TextBox11.Text)
    }

        ' Call the Crud function to execute the query and retrieve the result
        Dim dt As DataTable = Crud(sql, sqlParameters)

        ' Check if we received any rows
        If dt.Rows.Count > 0 Then
            ' Populate text boxes and combo boxes with the retrieved data
            With dt.Rows(0)
                TextBox3.Text = .Item("Lect").ToString()
                TextBox6.Text = .Item("Pnum").ToString()
                TextBox1.Text = .Item("mail").ToString()
                ComboBox2.Text = .Item("sta").ToString()
                ComboBox3.Text = .Item("lga").ToString()
                TextBox7.Text = .Item("homt").ToString()
                TextBox8.Text = .Item("addr").ToString()
                ComboBox6.Text = .Item("clas").ToString()
                ComboBox7.Text = .Item("leve").ToString()
                TextBox15.Text = .Item("adpost").ToString()
                ComboBox8.Text = .Item("dept").ToString()

                ' Load the image into PictureBox
                Dim picPath As String = Path.Combine(Application.StartupPath, "pic", .Item("pic").ToString())
                If File.Exists(picPath) Then
                    PictureBox1.Image = Image.FromFile(picPath)
                Else
                    MsgBox("Image not found.", vbExclamation, "CA")
                End If
            End With
        Else
            MsgBox("Invalid Staff ID", vbCritical, "CA")
        End If
    End Sub

    Private Sub Button5_Click(sender As Object, e As EventArgs) Handles Button5.Click
        ' Prepare the SQL update statement
        Dim sql As String = "UPDATE Lect SET " &
                        "Lect = @Lect, " &
                        "Pnum = @Pnum, " &
                        "mail = @Mail, " &
                        "sta = @Sta, " &
        ...
        New SqlParameter("@Leve", ComboBox7.Text),
