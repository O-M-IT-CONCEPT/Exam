...
Public Class Form13
    ...
    Private Sub Button5_Click(sender As Object, e As EventArgs) Handles Button5.Click
        If TextBox3.Text = "" Or TextBox6.Text = "" Or TextBox1.Text = "" Or ComboBox2.Text = "Select" Or ComboBox3.Text = "Select" Or TextBox7.Text = "" Or TextBox8.Text = "" Or ComboBox6.Text = "Select" Or ComboBox6.Text = "Select" Or TextBox15.Text = "" Or ComboBox8.Text = "Select" Or TextBox9.Text = "" Then
            MsgBox("FAILED", vbCritical, "CA")
        Else
            ' SQL Update query
            Dim sql As String = "UPDATE Lect SET Lect=@Lect, Pnum=@Pnum, mail=@mail, sta=@sta, lga=@lga, homt=@homt, addr=@addr, clas=@clas, cclas=@cclas, leve=@leve, adpost=@adpost, dept=@dept, pswd=@pswd WHERE staffid=@staffid"

            ' Parameters for the SQL query
            Dim sqlParameters As New List(Of SqlParameter) From {
            New SqlParameter("@Lect", TextBox3.Text),
            ...
            New SqlParameter("@cclas", ComboBox6.Text),
            New SqlParameter("@leve", "nil"), ' Or replace with a proper value if needed
            New SqlParameter("@adpost", TextBox15.Text),
            New SqlParameter("@dept", ComboBox8.Text),
            New SqlParameter("@pswd", TextBox9.Text),
            New SqlParameter("@staffid", Form9.lblid.Text)
        }

            ' Execute the update using the Crud function
            Crud(sql, sqlParameters)

            MsgBox("SUCCESS", vbInformation, "CA")
    ...
    Private Sub Form13_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ' SQL Select query to load lecturer information based on staffid
        Dim sql As String = "SELECT Lect, Pnum, mail, sta, lga, homt, addr, clas, cclas, adpost, dept, pic FROM Lect WHERE staffid=@staffid"
        ...
        ' Populate fields with the selected lecturer's information
        If dt.Rows.Count > 0 Then
            Dim row As DataRow = dt.Rows(0)
            TextBox3.Text = row("Lect").ToString()
            TextBox6.Text = row("Pnum").ToString()
            TextBox1.Text = row("mail").ToString()
            ComboBox2.Text = row("sta").ToString()
            ComboBox3.Text = row("lga").ToString()
            TextBox7.Text = row("homt").ToString()
            TextBox8.Text = row("addr").ToString()
            ComboBox6.Text = row("clas").ToString()
            ComboBox6.Text = row("cclas").ToString()
            TextBox15.Text = row("adpost").ToString()
            ComboBox8.Text = row("dept").ToString()

            ' Load lecturer's picture
            Dim picPath As String = Application.StartupPath & "\pic\" & row("pic").ToString()
            If System.IO.File.Exists(picPath) Then
                PictureBox1.Image = Image.FromFile(picPath)
            End If
        End If

        ' Populate ComboBox2 with Nigerian states
        With ComboBox2
            .Items.AddRange(New String() {"ABUJA", "ABIA", "ADAMAWA", "AKWA IBON", "ANAMBRA", "BAUCHI", "BAYELSA", "BENUE", "BORNO", "CROSS RIVER", "DELTA", "EDO", "EBONYI", "EKITI", "ENUGU", "GOMBE", "IMO", "JIGAWA", "KADUNA", "KANO", "KATSINA", "KEBBI", "KOGI", "KWARA", "LAGOS", "NIGER", "OGUN", "ONDO", "OSUN", "OYO", "NASSARAWA", "PLATEAU", "RIVERS", "SOKOTO", "TARABA", "YOBE", "ZAMFARA"})
        End With
    End Sub
