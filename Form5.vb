...
Public Class Form5
    Private Sub Form5_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ' Populate ComboBox with states
        With ComboBox2
            .Items.Clear() ' Clear existing items before adding new ones
            .Items.AddRange(New String() {
            "ABUJA", "ABIA", "ADAMAWA", "AKWA IBON", "ANAMBRA",
            "BAUCHI", "BAYELSA", "BENUE", "BORNO", "CROSS RIVER",
            "DELTA", "EDO", "EBONYI", "EKITI", "ENUGU", "GOMBE",
            "IMO", "JIGAWA", "KADUNA", "KANO", "KATSINA", "KEBBI",
            "KOGI", "KWARA", "LAGOS", "NIGER", "OGUN", "ONDO",
            "OSUN", "OYO", "NASSARAWA", "PLATEAU", "RIVERS",
            "SOKOTO", "TARABA", "YOBE", "ZAMFARA"
        })
        End With

        ' Initialize variables
        Dim sql As String = "SELECT * FROM adminn"
        Dim dt As DataTable
        Dim sid As String
        Dim NUM As Integer

        Try
            ' Use the Crud function to get data
            dt = Crud(sql, Nothing)

            ' Generate a new ID
            If dt.Rows.Count > 0 Then
                NUM = dt.Rows(dt.Rows.Count - 1).Field(Of Integer)("ID") ' Get the last ID
                NUM += 1
            Else
                NUM = 1 ' Start from 1 if there are no records
            End If

            sid = "FPO/ADM/" & Today.ToString("yyyy-MM-dd") & "/" & Format(NUM, "000000")
            TextBox2.Text = sid
    ...
    Private Sub Button1_Click(sender As Object, e As EventArgs) Handles Button1.Click
       ...
       ComboBox2.Text = "Select" OrElse
       ComboBox3.Text = "Select" Then
        ...
        If TextBox9.Text <> TextBox10.Text Then
            MsgBox("Passwords do not match", vbCritical, "CA")
            Return
        End If

        ' Save the picture
        PictureBox1.Image.Save(Application.StartupPath & "\pic\" & TextBox6.Text & TextBox1.Text & ".jpg")

        ' Prepare SQL insert command
        Dim sql As String = "INSERT INTO adminn (fnam, pno, mail, state, lga, hometwn, addr, pswd, pic, admid, utype) " &
                        "VALUES (@fnam, @pno, @mail, @state, @lga, @hometwn, @addr, @pswd, @pic, @admid, @utype)"

        ' Create SQL parameters
        Dim sqlParameters As New List(Of SqlParameter) From {
        New SqlParameter("@fnam", TextBox3.Text),
        ...
        New SqlParameter("@hometwn", TextBox7.Text),
        New SqlParameter("@addr", TextBox8.Text),
        New SqlParameter("@pswd", TextBox9.Text),
        New SqlParameter("@pic", TextBox6.Text & TextBox1.Text & ".jpg"),
        New SqlParameter("@admid", TextBox2.Text),
        New SqlParameter("@utype", "Admin")
    }

        Try
            ' Call the Crud function to execute the insert command
            Crud(sql, sqlParameters)
            MsgBox("SAVED SUCCESSFULLY", vbInformation, "CA")
        Catch ex As Exception
            MsgBox("An error occurred: " & ex.Message, vbCritical, "CA")
        End Try
    End Sub
