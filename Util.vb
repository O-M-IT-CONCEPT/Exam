...
Public Class Util
	...
	Public Function InitializeGrFinger() As Integer
        ...
        Return GrFinger.CapInitialize(AddressOf frmCapture.GrFinger_SensorPlugUnPlug)
	End Function
    ...
    Public Sub PrintBiometricDisplay(ByVal biometricDisplay As Boolean, ByVal context As Integer)
        ...
        If handle <> 0 Then
            _pbPic.Image = System.Drawing.Bitmap.FromHbitmap(New IntPtr(handle))
            _pbPic.Update()
        End If
        ' release screen HDC
        ReleaseDC(0, hdc)
	...
	Function ExtractTemplate() As Integer
        ...
        ret = GrFinger.Extract(raw.img, raw.width, raw.height, raw.res, template.tpt, template.Size, GrFinger.GR_DEFAULT_CONTEXT)
	...
	Public Function Identify(ByRef score As Integer) As Integer
        ...
        ret = GrFinger.IdentifyPrepare(tmpTpt, GrFinger.GR_DEFAULT_CONTEXT)
		' error?
		...
		For i = 1 To templates.Length
            ...
            If Not (templates(i - 1).template Is Nothing) Then
                ...
                ret = GrFinger.Identify(tempTpt, score, GrFinger.GR_DEFAULT_CONTEXT)
            End If
	...
	Public Function Verify(ByVal id As Integer, ByRef score As Integer) As Integer
        ...
        Return GrFinger.Verify(tempTpt, tptref, score, GrFinger.GR_DEFAULT_CONTEXT)
	End Function

	' Show GrFinger version and type
	Public Sub MessageVersion()
        Dim majorVersion As Byte = 0
        Dim minorVersion As Byte = 0
        Dim result As Integer
        Dim vStr As String = ""

        result = GrFinger.GetGrFingerVersion(majorVersion, minorVersion)
        If result = GrFinger.GRFINGER_FULL Then vStr = "FULL"
        If result = GrFinger.GRFINGER_LIGHT Then vStr = "LIGHT"
